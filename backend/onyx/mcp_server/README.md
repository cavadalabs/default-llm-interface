# Default LLM Interface MCP Server

## Overview

The Default LLM Interface MCP server allows LLMs to connect to your application instance and access its knowledge base and search capabilities through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/).

With the MCP Server, you can search your knowledgebase,
give your LLMs web search, and upload and manage documents in the application.

All access controls are managed within the main application.

### Authentication

Provide an application Personal Access Token or API Key in the `Authorization` header as a Bearer token.
...
```json
{
  "mcpServers": {
    "onyx": {
      "url": "https://[YOUR_APPLICATION_DOMAIN]:8090/",
      "transport": "http",
      "headers": {
        "Authorization": "Bearer YOUR_APPLICATION_TOKEN_HERE"
      }
    }
  }
}
...
Search the user's private knowledge base indexed in the application. Returns ranked documents with content snippets, scores, and metadata.
...
The MCP Server automatically launches with the local all-services task from the default launch.json.
...
4. Paste your application bearer token
5. Click **Connect**
...
- `API_SERVER_URL_OVERRIDE_FOR_HTTP_REQUESTS`: Optional override URL. If set, takes precedence over the protocol/host variables. Used for self-hosting the MCP server with a Cloud backend.
