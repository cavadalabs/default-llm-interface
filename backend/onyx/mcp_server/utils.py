...
def require_access_token() -> AccessToken:
    ...
    if not access_token:
        raise ValueError(
            "MCP Server requires an application access token to authenticate your request"
        )
    return access_token
...
async def get_indexed_sources(
    ...
    try:
        ...
        logger.error(
            "MCP Server: Failed to fetch indexed sources",
            exc_info=True,
        )
        raise
        ...
        logger.error(
            "MCP Server: Unexpected error fetching indexed sources",
            exc_info=True,
        )
...
async def get_accessible_document_sets(
    ...
    try:
        ...
        logger.error(
            "MCP Server: Failed to fetch document sets",
            exc_info=True,
        )
        raise
    except Exception as exc:
        logger.error(
            "MCP Server: Unexpected error fetching document sets",
            exc_info=True,
        )
