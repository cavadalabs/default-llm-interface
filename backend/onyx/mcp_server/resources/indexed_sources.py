"""Resources that expose metadata for the MCP server."""
...
@mcp_server.resource(
    ...
    description=(
        "Enumerate the user's document sources that are currently indexed in the application."
...
async def indexed_sources_resource() -> str:
    ...
    logger.info(
        "MCP Server: indexed_sources resource returning %s entries",
        len(sources),
    )
