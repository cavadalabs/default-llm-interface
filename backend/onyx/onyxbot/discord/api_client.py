"""Async HTTP client for communicating with API pods."""
...
class OnyxAPIClient:
    """Async HTTP client for sending chat requests to API pods.

    This client manages an aiohttp session for making non-blocking HTTP
    requests to the API server. It handles authentication with per-tenant
    ...
    async def send_chat_message(
        ...
        """Send a chat message to the API server and get a response.
